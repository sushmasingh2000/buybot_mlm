...
const Fund = () => {
  ...
  const handleFundRequestSubmit = async (e) => {
    ...
    try {
      ...
      const res = await apiConnectorPost(endpoint?.add_user_fund_request, payload);
      if (res?.data?.status === true) {
        toast(res.data?.message === "Request Successfully Done");
        setAmount('');
        setReceiptFile(null);
      } else {
  ...
  const handleCopyAddress = () => {
    navigator.clipboard.writeText(address?.m39_add || '')
        ...
        <div className="flex flex-col md:flex-row gap-10 w-full">
          <form onSubmit={handleFundRequestSubmit} className="flex flex-col gap-4 w-full ">
            <div className="flex flex-col">
              <label htmlFor="" className="text-sm font-medium text-gray-300 mb-1">
                Crypto Address
              </label>
              <input
          ...
          <div className="bg-gray-700 p-4 flex flex-col items-start w-full  rounded-md">
            ...
            <img src={address?.m39_qr} alt='' className='p-4 w-[250px] h-[250px]' />
            <label htmlFor="walletAddressDisplay" className="text-sm font-medium text-gray-300 mb-2">
